...
namespace PluginX
{
    class Utils : InterfaceUtils
    {
        ...
        public void httpRequest(IDictionary<string, string> info)
        {
            string url = info["url"];
            // HTTP web request
            ...
            {
                HttpWebRequest http = result.AsyncState as HttpWebRequest;
                if (http != null)
                {
                    try
                    {
                        WebResponse response = http.EndGetResponse(result);
